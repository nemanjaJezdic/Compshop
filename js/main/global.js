...
$(document).ready(function () {
    ...
    $('body').on('click', '.add-to-cart', function (e) {
        ...
        setTimeout(() => {
            ...
            $.toast({ text: 'Successfully added product to cart. <a href="/checkout.html">Go to cart</a>', position: 'bottom-center' });
        }, 1500);
    });
    /*price range*/
            ...
            subcategories = `
                    <div id="accordian-${category.id}" class="panel-collapse collapse">
                        <div class="panel-body">
                            <ul>
                                ${category.subcategories.map((c) => `<li id="${c.id}" ><a href="/shop.html?categoryId=${c.id}">${c.name}</a></li>`).join('')}
                            </ul>
                        </div>
                    </div>
                `;
